...
> trivial to update the `main` branch with single files to be tested
> against.
...
1. Checkout `main`, then find the path of the file.
2. Do a `git log -- <path-to-file>` which should result
   in a log of all edits made to the the `<path-to-file>`.
3. Pick the top commit from 2. and do:

       git checkout stripped
       git cherry-pick <commit-hash>

It is important in 2. that one picks the most recent change of the
file as that corresponds to the development of the test in `main`.
I.e. one could update all files for a test with a new version of the
code (say the older version of the code is not supported any more).
In this case it would be important to pick the latest version of the
test files.
